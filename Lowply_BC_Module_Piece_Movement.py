'''Lowply_BC_Module_Piece_Movement
Matthew Blumenschein, mmb36
this file validates potential moves as well as carries out state transformation'''
...
def is_frozen(location, board, side):
    '''returns if a given piece is frozen'''
    enemy = 1 - side
    enemy_freezer = 15
    if enemy == 0:
        enemy_freezer = 14
...
def move(start_pos, dest, state, dir):
    '''makes a move and returns a new state'''
    board = state.board
    #print(state.hash)
    piece = board[start_pos[0]][start_pos[1]]
    whose_move = state.whose_move
    new_state = bcs.BC_state(board, 1 - whose_move)
...
def update_board(start, dest, state, piece, dir, whose_move):
    '''updates the new board to reflect the action being taken'''
...
def remove_captured_pieces(start, dest, state, piece, dir, whose_move):
    '''removes the pieces that are captured during the currenr move'''
    board = state.board
    capture_space = None
    #print(hash)
...
def get_line(start, dest, board, dir):
    '''returns a line represent the spaces on the board a move will travel through'''
    line = []
    current_pos = start
...
def check_move(start, dest, piece, board, side, enemy, dir):
    '''function called by can_move to validate if the move is legal'''
    piece = PIECES[piece]
    line = get_line(start, dest, board, dir)
...
def leaper_can_capture(line, side, enemy, is_imitator=False):
    '''returns if the leaper will capture a piece during a given move'''
...
def leaper_capture_space(start, dest, board, dir, whose_side):
    '''returns location of pieces captured by the leaper'''
...
def line_is_empty(line):
    '''returns if a line is empty reflecting a valid move for every piece'''
    for p in line:
        if PIECES[p] != '-':
            return False
    return True
...
def pincer_capture(pincer_location, board, pincer_side, is_imitator=False):
    '''returns the location of pieces captured by the pincer'''
    enemy = 1 - pincer_side
    pincer_captures = []
    for i in range(4):
...
def is_adjacent_to_enemy(piece_location, board, side, dir, is_imitator=False):
    '''for the withdrawer returns the location of a captured piece if it exists'''
    enemy = 1 - side
...
def coordinator_capture(coord_pos, board, side, is_imitator=False):
    '''returns the location of pieces captured by the coordinator'''
    enemy =  1 - side
    king = 13
    if side == 0:
        king = 12
...
def chameleon_captures(start_pos, dest, board, side, dir):
    '''returns the pieces captured by the imitator'''
    enemy = 1 - side
    captured_pieces = []
